...
// Performa Sales — revenue/HPP yang sama, dikelompokkan per sales rep di Penawaran asalnya
// (lewat provenance quotation_line -> invoice_line -> delivery_order_line).
$salesSql = 'SELECT
...
<div class="card">
  <table class="data-table">
    <thead><tr><th>Sales</th><th class="num">Revenue</th><th class="num">HPP</th><th class="num">Gross Profit</th></tr></thead>
    <tbody>
      <?php foreach ($salesRows as $sr): ?>
        <tr>
          <td><?= htmlspecialchars($sr['sales_name']) ?></td>
          <td class="num">Rp <?= number_format((float) $sr['revenue'], 0, ',', '.') ?></td>
          <td class="num">Rp <?= number_format((float) $sr['cogs'], 0, ',', '.') ?></td>
          <td class="num">Rp <?= number_format((float) $sr['revenue'] - (float) $sr['cogs'], 0, ',', '.') ?></td>
        </tr>
      <?php endforeach; ?>
      <?php if (!$salesRows): ?><tr><td colspan="4" style="text-align:center; color:var(--ink-muted);">Belum ada data.</td></tr><?php endif; ?>
    </tbody>
  </table>
</div>
