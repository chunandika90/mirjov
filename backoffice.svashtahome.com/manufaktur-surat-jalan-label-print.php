...
// Foto barang (kalau ada) — dicari dari nama barang yang cocok di master produk.
$photoStmt = $pdo->prepare('SELECT pp.file_path FROM product_photos pp JOIN products p ON p.id=pp.product_id WHERE p.organization_id=? AND p.name=? ORDER BY pp.sort_order, pp.id LIMIT 1');
$photoFallbackStmt = $pdo->prepare('SELECT photo_path FROM products WHERE organization_id=? AND name=?');
foreach ($lines as &$ln) {
    $ln['photo'] = null;
    $photoStmt->execute([$org['organization_id'], $ln['product_name_snapshot']]);
    $row = $photoStmt->fetch();
    if ($row) {
        $ln['photo'] = $row['file_path'];
    } else {
        $photoFallbackStmt->execute([$org['organization_id'], $ln['product_name_snapshot']]);
...
$expandedLines = [];
foreach ($lines as $ln) {
    $labelCount = max(1, (int) ($ln['kemasan'] ?? 1));
...
<head>
<meta charset="utf-8">
<title>Label — Surat Jalan <?= htmlspecialchars($h['doc_number']) ?></title>
<style>
...
<body>
          ...
          <div class="label-cell">
            <div class="label-head">
              <div class="brand">
                ...
                <b><?= htmlspecialchars($h['warehouse_snapshot'] ?: 'MJT') ?></b>
              </div>
            ...
            <div class="label-body">
              <div class="label-fields">
                ...
                <div class="label-field"><span class="k">QTY</span>: <?= $ln['qty'] ? htmlspecialchars($ln['qty']) : '' ?></div>
                <div class="label-field"><span class="k">TUJUAN</span>: <?= htmlspecialchars($h['kepada_snapshot'] ?: '') ?></div>
              </div>
            ...
            <div class="label-sign">
              PEMBUAT,
              <div class="line">( <?= htmlspecialchars($h['driver_name'] ?: '') ?> )</div>
            </div>
          </div>
        <?php endforeach; ?>
  ...
  <?php endforeach; ?>
  <?php if (!$pages): ?><div class="page"><div style="padding:20px;">Gak ada barang.</div></div><?php endif; ?>
