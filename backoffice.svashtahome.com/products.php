    ...
    try {
        if ($action === 'save_product') {
            ...
            if ($categoryId) {
                ...
                $catStmt->execute([$categoryId, $org['organization_id']]);
                $catRow = $catStmt->fetch();
                $categoryName = $catRow['name'] ?? null;
                if (!$categoryName) { $categoryId = null; $subcategoryId = null; }
            }
            if ($subcategoryId) {
                $subStmt = $pdo->prepare('SELECT id FROM product_subcategories WHERE id=? AND organization_id=? AND category_id=?');
                $subStmt->execute([$subcategoryId, $org['organization_id'], $categoryId]);
                if (!$subStmt->fetchColumn()) $subcategoryId = null;
            }
            // Kalau kategori dipilih tapi sub kategori kosong, otomatis dibikinin/dipakein
            // Sub Kategori senama kategorinya — biar gak ada barang yang nyangkut tanpa
            // sub kategori beneran (konsisten sama Master Barang kanban).
            if ($categoryId && !$subcategoryId) {
                $defSub = $pdo->prepare('SELECT id FROM product_subcategories WHERE organization_id=? AND category_id=? AND name=?');
                $defSub->execute([$org['organization_id'], $categoryId, $categoryName]);
                $defSubId = $defSub->fetchColumn();
                if ($defSubId) {
                    $subcategoryId = (int) $defSubId;
                } else {
                    $seqStmt = $pdo->prepare('SELECT COUNT(*) c FROM product_subcategories WHERE organization_id=? AND category_id=?');
                    $seqStmt->execute([$org['organization_id'], $categoryId]);
                    $n = (int) $seqStmt->fetch()['c'] + 1;
                    $defCode = sprintf('%s-%03d', $catRow['code'], $n);
                    $pdo->prepare('INSERT INTO product_subcategories (organization_id, category_id, code, name) VALUES (?,?,?,?)')
                        ->execute([$org['organization_id'], $categoryId, $defCode, $categoryName]);
            ...
            if ($id > 0) {
                $pdo->prepare('UPDATE products SET name=?, unit=?, category=?, category_id=?, subcategory_id=?, material=?, item_type=?, collection=?, finishing=?, size=?, panjang=?, lebar=?, tinggi=?, tinggi_dudukan=?, tinggi_lengan=?, tinggi_sandaran=?, tinggi_kaki=?, extra_specs=? WHERE id=? AND organization_id=?')
                    ->execute([$name, $unit, $categoryName, $categoryId, $subcategoryId, $material, $itemType, $collection, $finishing, $size,
                ...
                $pdo->prepare('INSERT INTO products (organization_id, name, unit, category, category_id, subcategory_id, material, item_type, collection, finishing, size, panjang, lebar, tinggi, tinggi_dudukan, tinggi_lengan, tinggi_sandaran, tinggi_kaki, extra_specs) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)')
                    ->execute([$org['organization_id'], $name, $unit, $categoryName, $categoryId, $subcategoryId, $material, $itemType, $collection, $finishing, $size,
                        $dimensions['panjang'], $dimensions['lebar'], $dimensions['tinggi'], $dimensions['tinggi_dudukan'], $dimensions['tinggi_lengan'], $dimensions['tinggi_sandaran'], $dimensions['tinggi_kaki'],
...
$pcCategories = $pdo->prepare('SELECT id, name FROM product_categories WHERE organization_id=? ORDER BY name');
$pcCategories->execute([$org['organization_id']]);
$pcCategories = $pcCategories->fetchAll();
$pcSubcategories = $pdo->prepare('SELECT id, category_id, name FROM product_subcategories WHERE organization_id=? ORDER BY name');
$pcSubcategories->execute([$org['organization_id']]);
$pcSubcategories = $pcSubcategories->fetchAll();
$pcSubByCategory = [];
foreach ($pcSubcategories as $sc) { $pcSubByCategory[(int) $sc['category_id']][] = $sc; }
        ...
        <form method="post">
          ...
          <div class="field-row">
            <div class="field"><label>Kategori</label>
              <select name="category_id" class="pc-category-select">
                <option value="">— Pilih Kategori —</option>
                <?php foreach ($pcCategories as $c): ?><option value="<?= $c['id'] ?>"><?= htmlspecialchars($c['name']) ?></option><?php endforeach; ?>
              </select>
            </div>
            <div class="field"><label>Sub Kategori</label>
              <select name="subcategory_id" class="pc-subcategory-select">
    ...
    <?php else: ?>
      ...
      <div class="card" style="margin-bottom:16px;">
        ...
        <form method="post" id="spec-form" enctype="multipart/form-data">
          ...
          <div class="field-row">
            <div class="field"><label>Kategori</label>
              <select name="category_id" class="pc-category-select" data-selected-sub="<?= (int) ($editingProduct['subcategory_id'] ?? 0) ?>">
                <option value="">— Pilih Kategori —</option>
                <?php foreach ($pcCategories as $c): ?><option value="<?= $c['id'] ?>" <?= (int) $editingProduct['category_id'] === (int) $c['id'] ? 'selected' : '' ?>><?= htmlspecialchars($c['name']) ?></option><?php endforeach; ?>
              </select>
            </div>
            <div class="field"><label>Sub Kategori</label>
              <select name="subcategory_id" class="pc-subcategory-select">
        ...
        <?php else: ?>
          <div class="txn-info-strip">
            <div><span class="lbl">Kategori</span><?= htmlspecialchars($editingProduct['category'] ?: '—') ?></div>
...
var SUBCATEGORIES_BY_CATEGORY = <?= json_encode(array_map(function ($list) {
    return array_map(function ($s) { return ['id' => (int) $s['id'], 'name' => $s['name']]; }, $list);
}, $pcSubByCategory)) ?>;

function syncSubcategoryOptions(catSelect) {
  var subSelect = catSelect.closest('.field-row').querySelector('.pc-subcategory-select');
  var selectedSub = parseInt(catSelect.dataset.selectedSub || '0', 10);
  var subs = SUBCATEGORIES_BY_CATEGORY[catSelect.value] || [];
  subSelect.innerHTML = '<option value="">— Pilih Sub Kategori —</option>';
  subs.forEach(function (s) {
    var opt = document.createElement('option');
    opt.value = s.id;
    opt.textContent = s.name;
    if (s.id === selectedSub) opt.selected = true;
    subSelect.appendChild(opt);
  });
  catSelect.dataset.selectedSub = '';
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('.pc-category-select').forEach(function (el) {
    syncSubcategoryOptions(el);
    el.addEventListener('change', function () { syncSubcategoryOptions(el); });
  });
});
</script>
