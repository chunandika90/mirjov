...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$isOwner) {
        ...
        try {
            ...
            } elseif ($action === 'update_warehouse') {
                // Batasin anggota (selain Owner) cuma bisa liat transaksi (Surat Jalan/Saldo
                // Awal/Inventory) di 1 lokasi doang. NULL = gak dibatasin (liat semua lokasi).
                $membershipId = (int) ($_POST['membership_id'] ?? 0);
                $warehouseId = (int) ($_POST['warehouse_id'] ?? 0) ?: null;
                $pdo->prepare('UPDATE user_organization_roles SET warehouse_id=? WHERE id=? AND organization_id=?')
                    ->execute([$warehouseId, $membershipId, $org['organization_id']]);
                $flash = ['ok', 'Lokasi anggota diperbarui.'];
...
$members = $pdo->prepare(
    'SELECT uor.id AS membership_id, uor.status, uor.role_id, uor.warehouse_id, u.id AS user_id, u.name, u.email, r.name AS role_name,
       u.entity_type, u.subject_to_pph, w.name AS warehouse_name
     FROM user_organization_roles uor
     JOIN users u ON u.id = uor.user_id
     JOIN roles r ON r.id = uor.role_id
     LEFT JOIN warehouses w ON w.id = uor.warehouse_id
...
$uWarehouses = $pdo->prepare('SELECT id, name FROM warehouses WHERE organization_id=? AND deleted_at IS NULL ORDER BY is_default DESC, name');
$uWarehouses->execute([$org['organization_id']]);
$uWarehouses = $uWarehouses->fetchAll();
?>
...
<div class="card">
  <table class="data-table">
    <thead><tr><th>Nama</th><th>Email</th><th>Role</th><th>Lokasi</th><th>Tipe (Komisi)</th><th>Status</th><?php if ($isOwner): ?><th></th><?php endif; ?></tr></thead>
    <tbody>
      <?php foreach ($members as $m): ?>
        <tr>
          ...
          <td>
            <?php if ($m['role_name'] === 'Owner'): ?>
              <span class="pill owner">Semua Lokasi</span>
            <?php elseif ($isOwner): ?>
              <form method="post" onchange="this.submit();">
                <?= csrf_field() ?>
                <input type="hidden" name="action" value="update_warehouse">
                <input type="hidden" name="membership_id" value="<?= $m['membership_id'] ?>">
                <select name="warehouse_id" style="padding:4px 6px; border:1px solid var(--border); border-radius:4px; font-size:12px;">
                  <option value="">Semua Lokasi</option>
                  <?php foreach ($uWarehouses as $w): ?>
                    <option value="<?= $w['id'] ?>" <?= (int) $m['warehouse_id'] === (int) $w['id'] ? 'selected' : '' ?>><?= htmlspecialchars($w['name']) ?></option>
                  <?php endforeach; ?>
                </select>
              </form>
            <?php else: ?>
              <span class="pill"><?= $m['warehouse_name'] ? htmlspecialchars($m['warehouse_name']) : 'Semua Lokasi' ?></span>
            <?php endif; ?>
          </td>
          <td>
            <?php if ($isOwner): ?>
