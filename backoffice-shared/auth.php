...
// Banyak halaman require includes/header.php (yang langsung ngeluarin HTML sidebar/topbar)
// SEBELUM ngecek POST dan manggil header('Location: ...') buat redirect abis simpan.
// Tanpa buffering, begitu sidebar udah kekirim ke browser, header() telat dipanggil
// dan gagal diem-diem ("headers already sent") — user ngerasa "klik Simpan diem aja"
// padahal datanya kesimpen, cuma redirect-nya yang gak jalan. Buffer di sini biar
// header() masih bisa dipanggil kapan aja sebelum PHP beneran ngirim outputnya.
//
// PENTING: server ini punya php.ini output_buffering=4096, jadi ob_get_level() UDAH
// balik 1 dari awal request (buffer implisit bawaan PHP, bukan punya kita) — kalau
// di-guard `if (!ob_get_level())` kayak sebelumnya, ob_start() kita gak akan pernah
// kepanggil, dan buffer implisit yang cuma nampung 4096 byte itu yang tetep ngontrol,
// jadi begitu HTML sidebar lewat 4096 byte dia auto-flush duluan (headers kekirim)
// SEBELUM sempet ke-redirect. Makanya di sini HARUS selalu ob_start() baru (nested di
// atas buffer implisit itu gak masalah — buffer kita gak auto-flush sampe abis script).
ob_start();
...
{
    $stmt = db()->prepare(
        'SELECT o.id AS organization_id, o.legal_name, r.id AS role_id, r.name AS role_name, r.is_production_role
...
{
    $stmt = db()->prepare(
        'SELECT o.id AS organization_id, o.legal_name, r.id AS role_id, r.name AS role_name, r.is_production_role, uor.warehouse_id
         FROM user_organization_roles uor
         JOIN organizations o ON o.id = uor.organization_id
         JOIN roles r ON r.id = uor.role_id
...
/** True kalau role aktif ditandai sebagai role produksi/pabrik (lihat roles.php). */
function is_production_user(): bool
{
    $org = current_org();
    return !empty($org['is_production_role']);
}

/**
 * Balikin warehouse_id kalau anggota aktif dibatasin cuma boleh liat 1 lokasi, atau NULL
 * kalau gak dibatasin (bebas liat semua lokasi) — ini SELALU NULL buat role Owner, gak
 * peduli kolom warehouse_id-nya keisi apa kagak (Owner harus selalu liat semua).
 */
function user_location_restriction(): ?int
{
    $org = current_org();
    if (!$org || ($org['role_name'] ?? '') === 'Owner') return null;
    return isset($org['warehouse_id']) && $org['warehouse_id'] !== null ? (int) $org['warehouse_id'] : null;
}
